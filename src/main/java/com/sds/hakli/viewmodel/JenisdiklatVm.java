...
public class JenisdiklatVm {
	
	private Jenisdiklat objForm;
	private List<Jenisdiklat> objList;
	private String nama;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		grid.setRowRenderer(new RowRenderer<Jenisdiklat>() {

			@Override
			public void render(Row row, Jenisdiklat data, int index) throws Exception {
				row.getChildren().add(new Label(String.valueOf(index+1)));
				row.getChildren().add(new Label(data.getMrumpundiklat().getRumpundiklat()));
				...
				btEdit.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
					...
					public void onEvent(Event event) throws Exception {
						doAdd(data);
						BindUtils.postNotifyChange(JenisdiklatVm.this, "objForm");
					}
				});
						...
						Messagebox.show("Anda ingin menghapus data ini?", "Confirm Dialog", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new EventListener<Event>() {
							...
							public void onEvent(Event event)
								...
								if (event.getName().equals("onOK")) {
									try {
										String url = ConfigUtil.getConfig().getUrl_base() + ConfigUtil.getConfig().getEndpoint_jenisdiklat() + "/" + data.getId();
										...
										if (respAll.getCode() == 200) {
											...
											doRefresh();
											BindUtils.postNotifyChange(JenisdiklatVm.this, "*");
										} else {
		...
		try {
			String url = ConfigUtil.getConfig().getUrl_base() + ConfigUtil.getConfig().getEndpoint_rumpundiklat();
	...
	public void doReset() {
		...
		btAdd.setLabel("Tambah Jenis Diklat");
	...
	public void doRefresh() {
		try {
			String url = ConfigUtil.getConfig().getUrl_base()
					+ ConfigUtil.getConfig().getEndpoint_jenisdiklat();
	...
	@Command
	@NotifyChange("*")
	public void doAdd(Jenisdiklat obj) {
		if (obj != null) {
			isInsert = false;
			objForm = obj;
			...
			cbRumpundiklat.setValue(objForm.getMrumpundiklat().getRumpundiklat());
		} else if (btAdd.getLabel().equals("Tambah Jenis Diklat")) {
			isInsert = true;
			objForm = new Jenisdiklat();
			divForm.setVisible(true);
			btAdd.setLabel("Cancel");
			btAdd.setIconSclass("z-icon-reply");
	...
	public void doSave() {
		try {
			String url = ConfigUtil.getConfig().getUrl_base() + ConfigUtil.getConfig().getEndpoint_jenisdiklat();
			...
			if (isInsert && respAll.getCode() == 201) {
				Jenisdiklat data = (Jenisdiklat) mapper.readValue(mapper.writeValueAsString(respAll.getData()),
						Jenisdiklat.class);
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				String jenisdiklat = (String) ctx.getProperties("jenisdiklat")[0].getValue();
				
				if (jenisdiklat == null || "".equals(jenisdiklat.trim()))
	...
	public Jenisdiklat getObjForm() {
		return objForm;
	}

	public void setObjForm(Jenisdiklat objForm) {
