...
public class RumpundiklatVm {
	
	private Rumpundiklat objForm;
	private List<Rumpundiklat> objList;
	private String nama;
	...
	@Wire
	private Window winRumpundiklat;
	@Wire
	private Grid grid;
	@Wire
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		grid.setRowRenderer(new RowRenderer<Rumpundiklat>() {

			@Override
			public void render(Row row, Rumpundiklat data, int index) throws Exception {
				...
				btEdit.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
					...
					public void onEvent(Event event) throws Exception {
						doAdd(data);
						BindUtils.postNotifyChange(RumpundiklatVm.this, "objForm");
					}
				});
						...
						Messagebox.show("Anda ingin menghapus data ini?", "Confirm Dialog", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new EventListener<Event>() {
							...
							public void onEvent(Event event)
								...
								if (event.getName().equals("onOK")) {
									try {
										String url = ConfigUtil.getConfig().getUrl_base() + ConfigUtil.getConfig().getEndpoint_rumpundiklat() + "/" + data.getId();
										...
										if (respAll.getCode() == 200) {
											...
											doRefresh();
											BindUtils.postNotifyChange(RumpundiklatVm.this, "*");
										} else {
	...
	public void doReset() {
		...
		btAdd.setLabel("Tambah Rumpun Diklat");
	...
	public void doRefresh() {
		try {
			String url = ConfigUtil.getConfig().getUrl_base()
					+ ConfigUtil.getConfig().getEndpoint_rumpundiklat();
	...
	@Command
	@NotifyChange("*")
	public void doAdd(Rumpundiklat obj) {
		if (obj != null) {
			isInsert = false;
			objForm = obj;
		...
		} else if (btAdd.getLabel().equals("Tambah Rumpun Diklat")) {
			isInsert = true;
			objForm = new Rumpundiklat();
			divForm.setVisible(true);
			btAdd.setLabel("Cancel");
			btAdd.setIconSclass("z-icon-reply");
	...
	public void doSave() {
		try {
			...
			if (isInsert && respAll.getCode() == 201) {
				Rumpundiklat data = (Rumpundiklat) mapper.readValue(mapper.writeValueAsString(respAll.getData()),
						Rumpundiklat.class);
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				String rumpundiklat = (String) ctx.getProperties("rumpundiklat")[0].getValue();
				
				if (rumpundiklat == null || "".equals(rumpundiklat.trim()))
	...
	public Rumpundiklat getObjForm() {
		return objForm;
	}

	public void setObjForm(Rumpundiklat objForm) {
