...
public class RumpunVm {
	
	private Rumpun objForm;
	private List<Rumpun> objList;
	private String nama;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		grid.setRowRenderer(new RowRenderer<Rumpun>() {

			@Override
			public void render(Row row, Rumpun data, int index) throws Exception {
				...
				btEdit.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
					...
					public void onEvent(Event event) throws Exception {
						doAdd(data);
						BindUtils.postNotifyChange(RumpunVm.this, "objForm");
					}
				});
						...
						Messagebox.show("Anda ingin menghapus data ini?", "Confirm Dialog", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new EventListener<Event>() {
							...
							public void onEvent(Event event)
								...
								if (event.getName().equals("onOK")) {
									try {
										String url = ConfigUtil.getConfig().getUrl_base() + ConfigUtil.getConfig().getEndpoint_rumpun() + "/" + data.getId();
										...
										if (respAll.getCode() == 200) {
											...
											doRefresh();
											BindUtils.postNotifyChange(RumpunVm.this, "*");
										} else {
	...
	public void doRefresh() {
		try {
			String url = ConfigUtil.getConfig().getUrl_base()
					+ ConfigUtil.getConfig().getEndpoint_rumpun();
	...
	public void doAdd(Rumpun obj) {
		...
		} else if (btAdd.getLabel().equals("Tambah Rumpun")) {
			isInsert = true;
			objForm = new Rumpun();
			divForm.setVisible(true);
			btAdd.setLabel("Cancel");
	...
	public void doSave() {
		try {
			String url = ConfigUtil.getConfig().getUrl_base() + ConfigUtil.getConfig().getEndpoint_rumpun();
			...
			if (isInsert && respAll.getCode() == 201) {
				Rumpun data = (Rumpun) mapper.readValue(mapper.writeValueAsString(respAll.getData()),
						Rumpun.class);
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				String rumpun = (String) ctx.getProperties("rumpun")[0].getValue();
				
				if (rumpun == null || "".equals(rumpun.trim()))
	...
	public Rumpun getObjForm() {
		return objForm;
	}

	public void setObjForm(Rumpun objForm) {
