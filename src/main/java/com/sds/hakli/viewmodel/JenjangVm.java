...
public class JenjangVm {
	
	private Jenjang objForm;
	private List<Jenjang> objList;
	private String nama;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		grid.setRowRenderer(new RowRenderer<Jenjang>() {

			@Override
			public void render(Row row, Jenjang data, int index) throws Exception {
				...
				btEdit.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
					...
					public void onEvent(Event event) throws Exception {
						doAdd(data);
						BindUtils.postNotifyChange(JenjangVm.this, "objForm");
					}
				});
						...
						Messagebox.show("Anda ingin menghapus data ini?", "Confirm Dialog", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new EventListener<Event>() {
							...
							public void onEvent(Event event)
								...
								if (event.getName().equals("onOK")) {
									try {
										String url = ConfigUtil.getConfig().getUrl_base() + ConfigUtil.getConfig().getEndpoint_jenjang() + "/" + data.getId();
										...
										if (respAll.getCode() == 200) {
											...
											doRefresh();
											BindUtils.postNotifyChange(JenjangVm.this, "*");
										} else {
	...
	public void doRefresh() {
		try {
			String url = ConfigUtil.getConfig().getUrl_base()
					+ ConfigUtil.getConfig().getEndpoint_jenjang();
	...
	public void doAdd(Jenjang obj) {
		...
		} else if (btAdd.getLabel().equals("Tambah Jenjang")) {
			isInsert = true;
			objForm = new Jenjang();
			divForm.setVisible(true);
			btAdd.setLabel("Cancel");
	...
	public void doSave() {
		try {
			String url = ConfigUtil.getConfig().getUrl_base() + ConfigUtil.getConfig().getEndpoint_jenjang();
			...
			if (isInsert && respAll.getCode() == 201) {
				Jenjang data = (Jenjang) mapper.readValue(mapper.writeValueAsString(respAll.getData()),
						Jenjang.class);
	...
	public Jenjang getObjForm() {
		return objForm;
	}

	public void setObjForm(Jenjang objForm) {
