...
public class KepegawaianVm {
	
	private Kepegawaian objForm;
	private List<Kepegawaian> objList;
	private String nama;
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Component view) {
		...
		grid.setRowRenderer(new RowRenderer<Kepegawaian>() {

			@Override
			public void render(Row row, Kepegawaian data, int index) throws Exception {
				...
				btEdit.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
					...
					public void onEvent(Event event) throws Exception {
						doAdd(data);
						BindUtils.postNotifyChange(KepegawaianVm.this, "objForm");
					}
				});
						...
						Messagebox.show("Anda ingin menghapus data ini?", "Confirm Dialog", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new EventListener<Event>() {
							...
							public void onEvent(Event event)
								...
								if (event.getName().equals("onOK")) {
									try {
										String url = ConfigUtil.getConfig().getUrl_base() + ConfigUtil.getConfig().getEndpoint_kepegawaian() + "/" + data.getId();
										...
										if (respAll.getCode() == 200) {
											...
											doRefresh();
											BindUtils.postNotifyChange(KepegawaianVm.this, "*");
										} else {
	...
	public void doReset() {
		...
		btAdd.setLabel("Tambah Kepegawaian");
	...
	public void doRefresh() {
		try {
			String url = ConfigUtil.getConfig().getUrl_base()
					+ ConfigUtil.getConfig().getEndpoint_kepegawaian();
	...
	@Command
	@NotifyChange("*")
	public void doAdd(Kepegawaian obj) {
		if (obj != null) {
			isInsert = false;
			objForm = obj;
		...
		} else if (btAdd.getLabel().equals("Tambah Kepegawaian")) {
			isInsert = true;
			objForm = new Kepegawaian();
			divForm.setVisible(true);
			btAdd.setLabel("Cancel");
			btAdd.setIconSclass("z-icon-reply");
	...
	public void doSave() {
		try {
			...
			if (isInsert && respAll.getCode() == 201) {
				Kepegawaian data = (Kepegawaian) mapper.readValue(mapper.writeValueAsString(respAll.getData()),
						Kepegawaian.class);
	...
	public Validator getValidator() {
		return new AbstractValidator() {
			...
			public void validate(ValidationContext ctx) {
				String kepegawaian = (String) ctx.getProperties("kepegawaian")[0].getValue();
				
				if (kepegawaian == null || "".equals(kepegawaian.trim()))
	...
	public Kepegawaian getObjForm() {
		return objForm;
	}

	public void setObjForm(Kepegawaian objForm) {
